...
impl Spatial {
    pub fn new(soldiers: &[Soldier]) -> Self {
        ...
        for (i, s) in soldiers.iter().enumerate() {
            buckets.entry(Self::cell(s.position)).or_default().push(i);
        }
        Self { buckets }
    }
    fn cell(p: Point) -> (i32, i32) {
        ((p.x / 100.0).floor() as i32, (p.y / 100.0).floor() as i32)
    }
    pub fn near(&self, p: Point, radius: f64) -> Vec<usize> {
        ...
        for y in y0..=y1 {
            for x in x0..=x1 {
                if let Some(v) = self.buckets.get(&(x, y)) {
                    out.extend(v);
                }
            }
        }
