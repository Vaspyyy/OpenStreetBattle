...
impl Navigation {
    pub fn new(map: &Map, cell: f64) -> Result<Self, WorldError> {
        ...
        if !cell.is_finite() || cell < 1.0 {
            return Err(WorldError::Invalid(
                "navigation cell must be at least one metre".into(),
            ));
        }
        ...
        if width.saturating_mul(height) > 1_000_000 {
            return Err(WorldError::Invalid(
                "navigation exceeds one million cells; use a smaller extract".into(),
            ));
        }
        let mut n = Self {
            width,
            ...
            cell,
            min: map.bounds.min,
            free: vec![false; width * height],
        };
        for i in 0..n.free.len() {
            n.free[i] = map.walkable(n.center(i));
        }
        Ok(n)
    }
    fn center(&self, i: usize) -> Point {
        Point::new(
            self.min.x + (i % self.width) as f64 * self.cell + self.cell * 0.5,
            self.min.y + (i / self.width) as f64 * self.cell + self.cell * 0.5,
        )
    }
    fn index(&self, p: Point) -> Option<usize> {
        ...
        (x >= 0 && y >= 0 && x < self.width as isize && y < self.height as isize)
            .then_some(y.max(0) as usize * self.width + x.max(0) as usize)
    }
    fn closest_connected(&self, map: &Map, p: Point) -> Option<usize> {
        let initial = self.index(p)?;
        if self.free[initial] && map.clear_segment(p, self.center(initial)) {
            return Some(initial);
        }
        let (x, y) = (
            (initial % self.width) as isize,
            (initial / self.width) as isize,
        );
    ...
    pub fn nearest_walkable(&self, p: Point) -> Option<Point> {
        self.free
            ...
            .filter(|(_, f)| **f)
            .min_by(|(a, _), (b, _)| {
                p.distance(self.center(*a))
                    .total_cmp(&p.distance(self.center(*b)))
                    .then(a.cmp(b))
            })
            .map(|(i, _)| self.center(i))
    }
    pub fn path(&self, map: &Map, start: Point, goal: Point) -> Vec<Point> {
        if !map.walkable(start) || !map.walkable(goal) {
            return Vec::new();
        }
        if map.clear_segment(start, goal) {
            return vec![goal];
        }
        let (Some(s), Some(g)) = (
            self.closest_connected(map, start),
            self.closest_connected(map, goal),
        ) else {
            return Vec::new();
        };
        ...
        costs[s] = 0;
        open.push(Reverse((heuristic(s), 0u32, s)));
        while let Some(Reverse((_, cost, i))) = open.pop() {
            ...
            if i == g {
                ...
                while next < rev.len() {
                    ...
                    for (j, &point) in rev.iter().enumerate().skip(next) {
                        if map.clear_segment(from, point) {
                            furthest = j;
                        } else {
                            break;
                        }
                    }
                    result.push(rev[furthest]);
                    from = rev[furthest];
                    next = furthest + 1;
                }
                return result;
            ...
            for dy in -1isize..=1 {
                for dx in -1isize..=1 {
                    ...
                    if dx != 0
                        && dy != 0
                        && (!self.free[y as usize * self.width + nx as usize]
                            || !self.free[ny as usize * self.width + x as usize])
                    {
                        continue;
                    }
                    if !map.clear_segment(self.center(i), self.center(j)) {
                        continue;
                    }
                    ...
                    if nc < costs[j] {
                        costs[j] = nc;
                        parents[j] = i;
                        open.push(Reverse((nc + heuristic(j), nc, j)));
                    }
                }
            }
        }
...
mod tests {
    ...
    fn routes_around_a_building() {
        let m = Map::demo();
        let n = Navigation::new(&m, 10.0).unwrap();
        let mut p = Point::new(350.0, 300.0);
        let target = Point::new(550.0, 300.0);
        let path = n.path(&m, p, target);
        assert!(path.len() > 1);
        for q in path {
            assert!(m.clear_segment(p, q));
            p = q;
        }
        assert_eq!(p, target);
    }
    #[test]
    fn does_not_route_into_building() {
        let m = Map::demo();
        let n = Navigation::new(&m, 10.0).unwrap();
        assert!(
            n.path(&m, Point::new(350.0, 300.0), Point::new(450.0, 300.0))
                .is_empty()
        );
    }
    #[test]
    fn direct_open_route() {
        let m = Map::demo();
        let n = Navigation::new(&m, 10.0).unwrap();
        assert_eq!(
            n.path(&m, Point::new(10.0, 10.0), Point::new(100.0, 10.0)),
            vec![Point::new(100.0, 10.0)]
        );
    }
    #[test]
    fn invalid_resolution_rejected() {
        assert!(Navigation::new(&Map::demo(), 0.0).is_err());
    }
}
