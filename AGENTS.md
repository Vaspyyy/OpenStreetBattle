...
- Respect the release ladder. Fields/stubs are not completed features. The battle draws authoritative offline geometry; the optional World view uses MapLibre Native Vulkan. Read docs/REAL_WORLD.md. Do not claim campaign scale, real-world realism or GPU-driver coverage without measurements/tests.
...
- Native live-map changes must keep the checksum-pinned C artifact and Rust FFI revision aligned. Unsafe handle code is isolated to native/osb-map-native/src/vulkan.rs; do not introduce it into simulation crates.
- Map network access is explicit and bounded. Never make public geodata services the campaign tick backend. Geography failures must preserve the active battle.
