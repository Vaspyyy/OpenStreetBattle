...
#[command(
    version,
    about = "OpenStreetBattle deterministic headless simulation. No display, GPU or API key required."
)]
struct Args {
...
fn parse_duration(text: &str) -> Result<u64, String> {
    let split = text
        ...
        .unwrap_or(text.len());
    if split == 0 {
        return Err("duration must start with a non-negative integer".into());
    }
    let number = text[..split]
        .parse::<u64>()
        .map_err(|_| "duration integer overflow")?;
    let multiplier = match &text[split..] {
        ...
        "d" => 86400,
        "w" => 604800,
        _ => return Err("use seconds, minutes, hours, days or weeks: 30s, 2m, 1h, 7d, 1w".into()),
    };
    number
        .checked_mul(multiplier)
        .and_then(|s| s.checked_mul(TICKS_PER_SECOND))
        .ok_or_else(|| "duration overflows simulation clock".into())
}
fn main() -> Result<(), Box<dyn Error>> {
    let args = Args::parse();
    if let Some(path) = args.write_demo {
        fs::write(path, serde_json::to_string_pretty(&Scenario::demo())?)?;
        return Ok(());
    }
    if args.benchmark {
    ...
    for _ in 0..args.duration {
        sim.step()?;
        if let Some(output) = event_output.as_mut() {
            for e in sim.events() {
                if e.sequence > last_event {
...
mod tests {
    ...
    #[test]
    fn durations() {
        assert_eq!(parse_duration("2h").unwrap(), 72000);
        assert_eq!(parse_duration("0s").unwrap(), 0);
        assert_eq!(parse_duration("1w").unwrap(), 6048000);
    }
    #[test]
    fn bad_duration() {
        for s in ["-1h", "1.5h", "forever", "999999999999999999999d", ""] {
